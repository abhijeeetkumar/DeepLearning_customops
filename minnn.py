    ...
    def accumulate_grad_sparse(self, gs: List[Tuple[int, np.ndarray]]) -> None:
        ...
        if(self.grad is None):
          ...
          for i in range(self.data.shape[0]):
            self.grad[i] = np.zeros_like(self.data[0])       
...
class OpLookup(Op):
    ...
    def forward(self, W_emb: Tensor, words):
        results = np.zeros((len(words), W_emb.shape[1]))
        for i, word in enumerate(words):
          results[i] = W_emb.data[word]
        
        t_results = Tensor(results)
        self.store_ctx(W_emb=W_emb, words=words, t_results=t_results)
        return t_results

    def backward(self):
      W_emb, words, t_results = self.get_ctx('W_emb', 'words', 't_results')
      if t_results.grad is not None:
        t_prime_results = np.zeros_like(W_emb.data)
        for i, embV in enumerate(t_results.data):
            #[0][0] because where() givex (x,y) of each feature in embedding vector
            t_prime_results[np.where(W_emb.data == embV)[0][0]] += t_results.grad[i]

      W_emb.accumulate_grad(t_prime_results)
...
class OpDot(Op):
    ...
    def forward(self, W_h_i : Tensor, h: Tensor):
        t_result = Tensor(W_h_i.data.dot(h.data))
        self.store_ctx(W_h_i=W_h_i, h=h, t_result=t_result)
        return t_result
    
    def backward(self):
      W_h_i, h, t_result = self.get_ctx('W_h_i', 'h', 't_result')

      if t_result.grad is not None:
        h.accumulate_grad(t_result.grad.dot( W_h_i.data)) # W*dzdy
        W_h_i.accumulate_grad((t_result.grad).reshape(2,1).dot(h.data.reshape(1,2)))
...
class OpTanh(Op):
    ...
    def forward(self, x : Tensor):
      t_result = Tensor(np.tanh(x.data))
      self.store_ctx(x=x, t_result=t_result)
      return t_result
    
    def backward(self):
      x, t_result = self.get_ctx('x', 't_result')
      t_prime_result = np.zeros_like(x.data)

      if t_result.grad is not None:
        for i, _ in enumerate(x.data):
          #(tanh(x))' = 1 - (tanh(x))^2
          t_prime_result[i] = t_result.grad[i] *(1. - t_result.data[i]**2)

      x.accumulate_grad(t_prime_result)  
...
class OpMax(Op):
    ...
    def forward(self, a : Tensor, axis=0.):
          t_result = Tensor(np.amax(a.data, axis=axis))
          self.store_ctx(a=a, t_result=t_result, axis=axis)
          return t_result

    def backward(self):
        a, t_result, axis = self.get_ctx('a', 't_result', 'axis')
        if t_result.grad is not None:
          t_prime_result = np.zeros_like(a.data)
          for col, row in enumerate(np.argmax(a.data, axis)):
            t_prime_result[row][col] = t_result.grad[col]
          a.accumulate_grad(t_prime_result)
...
class OpAvg(Op):
    ...
    def forward(self, x : Tensor, axis: int):
      t_result = Tensor(np.average(x.data,axis=axis))
      self.store_ctx(x=x, axis=axis, t_result=t_result)
      return t_result

    def backward(self):
      x, axis, t_result = self.get_ctx('x', 'axis', 't_result')
      t_prime_result = np.zeros_like(x.data)
      #t_prime_result : np.ndarray


      if t_result.grad is not None:
        #only works for avg in axis=0; need to expand for other axis values
        for col, val in enumerate(t_result.grad):
          val = val/x.data.shape[0]
          for row in range(x.data.shape[0]):
            t_prime_result[row][col] = val 

      #print(t_prime_result)
      x.accumulate_grad(t_prime_result)
